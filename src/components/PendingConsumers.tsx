...
import { DollarSign, Zap, Calculator, CheckCircle, AlertCircle } from 'lucide-react';
...
export default function PendingConsumers() {
  ...
  useEffect(() => {
    const loadKwhPrice = async () => {
      try {
        ...
        console.log('PendingConsumers - Carregando preço do kWh...');
        const price = await settingsService.getKwhPrice();
        console.log('PendingConsumers - Preço do kWh carregado:', price);
        setKwhPrice(price);
      } catch (error) {
        console.error('Erro ao carregar preço do kWh:', error);
      } finally {
  ...
  const onApprove = async (id: string) => {
    ...
    await refetch();
    alert('Consumidor aprovado com sucesso');
  };

  const onReject = async (id: string, reason?: string) => {
    await rejectMutate(() => clienteConsumidorService.reject(id, reason));
    await refetch();
    alert('Consumidor rejeitado');
  };

  // Calculate commission for a consumer
  const calculateCommission = (consumer: any) => {
    console.log('calculateCommission - Consumer:', consumer);
    console.log('calculateCommission - Representative:', consumer.Representative);
    console.log('calculateCommission - kwhPrice:', kwhPrice);
    console.log('calculateCommission - averageMonthlyConsumption:', consumer.averageMonthlyConsumption);
    
    if (!consumer.Representative) {
      console.log('calculateCommission - ERRO: Sem representante');
      return null;
    }
    
    if (!kwhPrice) {
      console.log('calculateCommission - ERRO: Sem preço do kWh');
      return null;
    }
    
    if (!consumer.averageMonthlyConsumption) {
      console.log('calculateCommission - ERRO: Sem consumo médio mensal');
      return null;
    }
    ...
    console.log('calculateCommission - Usando fórmula padrão: (', kwh, '* 0.865 *', kwhPrice, ') / 2');
    console.log('calculateCommission - Resultado bruto:', kwh * 0.865 * kwhPrice);
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {loading && (
                <tr><td colSpan={10} className="px-4 py-6 text-center text-slate-500">Carregando...</td></tr>
              )}
              {error && !loading && (
                <tr><td colSpan={10} className="px-4 py-6 text-center text-red-600">{error}</td></tr>
              )}
              {!loading && !error && consumers.length === 0 && (
                <tr><td colSpan={10} className="px-4 py-6 text-center text-slate-500">Nenhum pendente encontrado</td></tr>
              )}
              {consumers.map((c: any) => {
