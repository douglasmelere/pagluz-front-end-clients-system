...
import { Plus, Search, Factory, Edit, Trash2, CheckCircle, Wind, Droplet, Leaf, Users, BarChart3, Activity, X, Loader, Sun } from 'lucide-react';
      ...
      <header className="bg-gradient-to-r from-slate-800 via-slate-700 to-emerald-600 shadow-lg rounded-b-3xl overflow-hidden">
        <div className="w-full px-3 sm:px-4 md:px-6 lg:px-8 py-4 sm:py-6">
          <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
            <div className="text-white flex items-center gap-x-2 sm:gap-x-4">
              <div className="h-10 w-10 sm:h-12 sm:w-12 bg-white/20 rounded-xl flex items-center justify-center"><Factory className="h-6 w-6 sm:h-7 sm:w-7" /></div>
              <div>
                <h1 className="text-2xl sm:text-3xl font-bold">Clientes Geradores</h1>
                <p className="text-slate-200 text-sm sm:text-base">Gestão de usinas e fontes de energia</p>
              </div>
            </div>
            <button onClick={handleAddNew} className="bg-gradient-to-r from-green-500 to-emerald-500 text-white px-4 sm:px-6 py-2 sm:py-3 rounded-xl flex items-center gap-x-2 font-semibold hover:scale-105 transition-transform shadow-lg text-sm sm:text-base"><Plus className="h-4 w-4 sm:h-5 sm:w-5" />Novo Gerador</button>
          </div>
        </div>
      </header>
