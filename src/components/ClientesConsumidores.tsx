...
export default function ClientesConsumidores() {
  ...
  const handleGenerateCommissionForConsumer = async (consumerId: string) => {
    try {
      ...
      console.log('👤 Representative ID:', consumer.representativeId);
  ...
  const handleApproveConsumer = async (consumer: Consumer) => {
    try {
      console.log(`🔄 Approving consumer: ${consumer.name} (${consumer.id})`);
      ...
      console.error(`❌ Error approving consumer ${consumer.name}:`, error);
  ...
  const handleEdit = (cliente: Consumer) => {
    console.log('Edit button clicked for client:', cliente.id, cliente.name);
  ...
  const renderGeneratorIcon = (sourceType: string, className: string) => {
    switch (sourceType) {
      case 'SOLAR':
        return <PagluzLogo className={className} />;
      case 'WIND':
      ...
      <div className="max-w-full mx-auto px-4 py-8 space-y-8">
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="min-w-full" style={{ minWidth: '1600px', tableLayout: 'fixed' }}>
              <thead>
      ...
      {showModal && (
        <ConsumidorModal
          ...
          onSave={async (data, action) => {
            console.log('onSave called with:', { action, data });
            try {
              let successMessage = '';
              ...
              if (action === 'create') {
                ...
                if (_generateCommissions) {
                  console.log('🔄 Representative attached - Commission generation needed');
                  console.log('📊 Consumer data after update:', data);
                  console.log('🎯 Consumer ID:', data.id);
                  console.log('👤 Representative ID:', data.representativeId);
                ...
                console.log('Reallocating client...');
...
function ConsumidorModal({ 
  ...
  useEffect(() => {
    if (cliente) {
      console.log('Updating form data for client:', cliente.id);
      setFormData({
        name: cliente.name || '',
        documentType: cliente.documentType || DocumentType.CPF,
        cpfCnpj: cliente.cpfCnpj || '',
  ...
  const handleSubmit = () => {
    ...
    if (cliente) {
      ...
      console.log('Client update logic:', {
        originalStatus,
        newStatus,
        originalGeneratorId,
        newGeneratorId,
        originalRepresentativeId,
        newRepresentativeId,
        representativeWasAdded,
        representativeWasChanged
      });
      ...
      if (newStatus === ConsumerStatus.ALLOCATED && originalStatus === ConsumerStatus.ALLOCATED && newGeneratorId && newGeneratorId !== originalGeneratorId) {
        ...
        console.log('Representative added/changed - will generate commissions');
