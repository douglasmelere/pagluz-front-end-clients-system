...
export const commissionService = {
  ...
  async generateCommissionsForExistingConsumers(): Promise<{
    ...
    try {
      ...
      console.log('commissionService.generateCommissionsForExistingConsumers - Resultado:', result);
      return result;
    } catch (error) {
      console.error('commissionService.generateCommissionsForExistingConsumers - Erro:', error);
      throw error;
    }
  },
  ...
  async generateCommissionsForConsumer(consumerId: string, consumerData?: any): Promise<{
    ...
    console.log('commissionService.generateCommissionsForConsumer - Iniciando para:', consumerId);
    console.log('commissionService.generateCommissionsForConsumer - Dados do consumidor:', consumerData);
    try {
      ...
      console.log('commissionService.generateCommissionsForConsumer - Dados da requisição:', requestData);
      
      const result = await api.post('/consumers/generate-commissions-all', requestData);
      console.log('commissionService.generateCommissionsForConsumer - Resultado:', result);
      ...
      if (result && result.results && result.results.length > 0) {
        ...
        if (consumerResult) {
          console.log('commissionService.generateCommissionsForConsumer - Comissão encontrada para o consumidor:', consumerResult);
          return consumerResult;
        }
      }
      
      // Se não encontrou comissão específica, retornar status de não gerado
      console.log('commissionService.generateCommissionsForConsumer - Nenhuma comissão gerada para este consumidor');
      return {
        consumerId,
      ...
      console.error('commissionService.generateCommissionsForConsumer - Erro:', error);
      throw error;
    }
  }
};
